...
func WithSpawnHandler(ctx context.Context, h sparkwing.SpawnHandler) context.Context {
	return context.WithValue(ctx, sparkwing.RuntimePlumbing.SpawnHandler, h)
}
...
func SpawnHandlerFromContext(ctx context.Context) sparkwing.SpawnHandler {
	if h, ok := ctx.Value(sparkwing.RuntimePlumbing.SpawnHandler).(sparkwing.SpawnHandler); ok {
		return h
	}
	return nil
}
